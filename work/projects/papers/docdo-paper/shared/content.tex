...
Data extraction was performed by a single GPT-5.2 batch pass against the full text of each included PDF (full extraction prompt and machine-readable outputs provided in the Supplementary Materials and repository package). There was no independent human re-extraction; field values were taken as stated in the LLM output and spot-checked by the authors against the source PDFs but not formally adjudicated by a second reviewer. Study authors were not contacted for missing or unclear data; extraction relied solely on published information. For each study, we extracted:
