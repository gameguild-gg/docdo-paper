...
\subsection{Clinical Translation Needs}

The practical challenges encountered in developing surgical planning platforms, such as DocDo \cite{docdo2026}, motivated this work. Such platforms deliver interactive 3D organ visualization for preoperative planning to surgeons, tumor margin assessment, and vascular anatomy mapping. Today, these pipelines often rely on manual and semi-automatic segmentation, taking 15 to 45 minutes per structure and creating adoption bottlenecks. Published benchmarks show $>$80\% Dice accuracy, yet clinical deployment demands robust preprocessing for diverse scanners, failure detection, uncertainty quantification, mesh generation, and DICOM integration---aspects that require dedicated engineering beyond benchmark performance.

This work serves as a technology roadmap, identifying the engineering investments needed to bridge the benchmark-to-clinic gap. Our phased adoption strategy (Section~\ref{sec:recommendations}) starts with robust baselines (TotalSegmentator~\cite{wasserthal2023totalsegmentator}, nnU-Net~\cite{isensee2021nnunet}), adds uncertainty-guided quality assurance, then explores interactive refinement (MedSAM~\cite{ma2024medsam}) to reduce, but not eliminate, expert correction.
...
\textbf{Supplementary materials and repository.} All supplementary artifacts (S0--S12, including S9 analysis scripts) cited in this paper are openly available in the project repository at \href{https://github.com/gameguild-gg/docdo-paper}{\faGithub\ gameguild-gg/docdo-paper} under \path{artifacts/data/evidence/supplementary/}. To keep in-text references concise, we cite each item once by its identifier and type (e.g., \emph{Supplementary Protocol~S3}, \emph{Supplementary Data~S5}, \emph{Supplementary Report~S6}); a complete index mapping each identifier to its filename and contents is provided in the Data Availability section.

\subsection{Search Strategy}

Three bibliographic databases were searched programmatically via their public APIs: PubMed (NCBI Entrez E-utilities), arXiv (official arXiv API), and Semantic Scholar (Graph API v1). Searches ran from November 15, 2025 to January 10, 2026, covering publications from 2015 onward. Closed-access indexes (IEEE Xplore, ACM Digital Library, Scopus) were not queried because no API access was available within the project budget; relevant papers indexed exclusively in those services were captured indirectly via Semantic Scholar's cross-publisher coverage and via the citation-tracking step described below.
...
OR "CT"[tiab])\\
AND ("image segmentation"[MeSH]\\
OR "segmentation"[tiab])\\
AND ("3D" OR "volumetric" OR "organ"\\
OR "multi-organ" OR "abdominal")\\
AND ("deep learning"[MeSH]\\
OR "neural network*" OR "convolutional"\\
OR "U-Net" OR "transformer"\\
OR "encoder-decoder"))\\
AND 2015:2026[dp] AND English[la]
\end{quote}

Equivalent queries for arXiv (using the \texttt{cat:cs.CV} and \texttt{cat:eess.IV} category filters with title/abstract keyword combinations) and Semantic Scholar (using the \texttt{/paper/search} endpoint with topical keyword strings) were constructed using each API's native syntax. Complete query strings for all three databases are provided in Supplementary Protocol~S3.
...
\begin{tikzpicture}[node distance=0.5cm,
    ...
    % Identification
    \node[box] (db) {Records from databases (n = 2,821)\\PubMed, arXiv, Semantic Scholar\\(after DOI + title deduplication)};
...
\textbf{Methodology Note:} We used an agentic screening pipeline: Elasticsearch pre-filtering followed by GPT-based consensus screening (GPT-4o-mini 3-vote unanimous-INCLUDE rule with GPT-5-nano cross-validation and GPT-5.2 tiebreaker) and a final GPT-5.2 full-text screening pass, with a human validation audit on a 10\% stratified sample (n=64 from 638 ES-filtered papers) reviewed by two domain experts blind to AI decisions and a third reviewer for discrepancies (AI--human agreement 96.4\%, $\kappa = 0.89$; 3 false negatives recovered; see Supplementary Report~S6). Data extraction and quality scoring were performed by a single GPT-5.2 batch pass using a predefined rubric described in Supplementary Protocol~S4 without independent human re-extraction or re-scoring. This represents structured survey methodology with AI-assisted screening rather than a prospective dual-reviewer systematic review; the absence of human re-extraction is acknowledged as a limitation in Section~\ref{sec:limitations}.

\textbf{Borderline exclusions:} Several studies initially appeared to meet inclusion criteria but were ultimately excluded with individual justification: Nikolov et al.\ \cite{nikolov2021clinically} was excluded because the full-text assessment revealed a focus on clinical validation methodology rather than presenting a novel segmentation architecture; Han et al.\ \cite{han2023scribble} was excluded as it addressed weakly-supervised segmentation using scribble annotations without reporting standard benchmark results on included datasets; studies evaluating only 2D slice-by-slice methods (n=2) lacked volumetric context despite targeting CT organs; and three studies reporting results exclusively on private institutional datasets without external validation were excluded per criterion EC3. Complete full-text screening decisions with individual exclusion reasons for all assessed studies are provided in Supplementary Data~S5. The full AI-assisted screening protocol (model versions, prompts, voting rules, and validation procedure) is documented in Supplementary Protocol~S4.
...
Data extraction was performed by a single GPT-5.2 batch pass against the full text of each included PDF (full extraction prompt in Supplementary Protocol~S4; machine-readable outputs in Supplementary Data~S7). There was no independent human re-extraction; field values were taken as stated in the LLM output and spot-checked by the authors against the source PDFs but not formally adjudicated by a second reviewer. Study authors were not contacted for missing or unclear data; extraction relied solely on published information. For each study, we extracted:
...
Study quality was scored by an LLM-based agentic pipeline (single GPT-5.2 batch call against the full text of each included PDF; rubric and prompt documented in Supplementary Protocol~S4) rather than by independent human reviewers. We note that this is a \emph{quality assessment framework}, not a formal risk-of-bias tool (e.g., ROBINS-I, Newcastle-Ottawa Scale). A formal risk-of-bias tool was not adopted because the included studies report diagnostic accuracy metrics from computational experiments rather than clinical intervention outcomes, rendering standard clinical RoB instruments inapplicable. The rubric comprised \textbf{15 questions, each scored 0--2 (0 = No/Poor, 1 = Partial, 2 = Yes/Good)}, grouped in three dimensions of 5 questions and 10 points each, yielding a total score of 0--30:
\begin{itemize}
    \item \textbf{Dataset quality} (5 questions, 0--10): dataset size adequacy, source description, train/validation/test split clarity, ground-truth annotation process, external validation on an independent dataset.
    \item \textbf{Methodology quality} (5 questions, 0--10): network architecture completeness, training reproducibility (hyperparameters, optimiser), loss-function justification, pre/post-processing description, public availability of code or trained model.
    \item \textbf{Evaluation quality} (5 questions, 0--10): use of multiple evaluation metrics (Dice plus at least one other), statistical analysis (confidence intervals, $p$-values), comparison against state-of-the-art baselines, per-organ rather than only aggregate results, and discussion of failure cases or limitations.
\end{itemize}
Studies with total score $\geq$24 were rated high quality; 15--23 as moderate; $<$15 as low. This rubric constitutes the study-level quality assessment instrument used in this review, applied in place of standard risk-of-bias tools (ROBINS-I, Cochrane RoB~2) that are designed for clinical study designs not applicable to computational benchmarking. Because scoring was performed by a single LLM pass without independent human re-scoring, inter-rater reliability statistics are not reported; the per-study rubric output is described in Section~\ref{sec:qa-results}.
...
\textbf{Sensitivity analyses:} To assess robustness, we conducted one post-hoc sensitivity analysis (not pre-specified in the review protocol): restricting findings to the 28 included studies whose primary identifier was a journal/conference DOI rather than an arXiv preprint identifier (operationalised in the supplementary extraction metadata) to confirm that architectural rankings and key conclusions remained consistent.
...
A comprehensive table listing all 52 included studies with their individual characteristics (author, year, architecture type, dataset(s), target organs, best reported Dice, code availability, and quality rating) is provided in Supplementary Data~S2. Below we summarize aggregate patterns.

Our survey of 52 studies reveals: \textbf{Architecture distribution:} CNNs dominated (98.1\%), with 3D U-Net~\cite{cicek20163dunet} variants most prevalent (82.7\%). True 3D volumetric processing was employed in 98.1\% of studies. \textbf{Segmentation scope:} Multi-organ (65.4\%) vs.\ single-organ (34.6\%). Most targeted organs: liver (n=29), kidney (n=26), bladder (n=20), spleen (n=17), pancreas (n=16). \textbf{Implementation:} TensorFlow/Keras (36.5\%), PyTorch (30.8\%), with a shift toward PyTorch in recent publications due to MONAI~\cite{monai2022}. Combined Dice+CE loss with Adam optimizer was predominant.
...
\textbf{Segmentation scope:} Multi-organ segmentation was addressed by 34 studies (65.4\%), while 18 studies (34.6\%) focused on single-organ segmentation. The most frequently targeted organs (substring match against the extracted organ-target field in the supplementary data table) were: liver (n=29), kidney/renal (n=26), bladder (n=20), spleen (n=17), pancreas (n=16), and stomach (n=16).

\textbf{Implementation patterns:} TensorFlow/Keras was the most common framework, used in 19 studies (36.5\%), followed by PyTorch in 16 studies (30.8\%), and Caffe in 4 studies (7.7\%). The remaining 13 studies (25.0\%) used other frameworks (e.g., Theano/Lasagne, NiftyNet, MATLAB-based pipelines) or did not report. The most common loss functions were cross-entropy and Dice loss, often used in combination (nnU-Net~\cite{isensee2021nnunet} default). Adam optimizer was predominant. The shift toward PyTorch is notable in more recent publications (2021--2024), likely due to the MONAI~\cite{monai2022} framework's PyTorch foundation.
...
\bottomrule
\multicolumn{5}{l}{\footnotesize Full per-study data available in Supplementary Data~S12.}
\end{tabular}
\end{table}
...
\textbf{Temporal distribution:} Publication years span 2017--2025 (50 of 52 studies have an explicit year in the extraction; one row carries an anomalous \texttt{year=2026} value for a 2025 BMC Medical Imaging article and one row has a blank year field). Peak activity was in 2020 (11 papers, 21.2\%) and 2022 (8 papers, 15.4\%). This aligns with the maturation of 3D CNN architectures and the emergence of transformer-based methods. Early works like Zhou et al.\ \cite{zhou2017deep} pioneered 2D-to-3D fusion approaches, while recent studies like Luo et al.\ \cite{luo2024raos} focus on robustness evaluation for clinical deployment.
...
\textbf{Reproducibility status:} Among 52 studies, 28 (53.8\%) had a journal/conference DOI as their primary identifier in the supplementary extraction table (the remaining 24 are arXiv-only identifiers, of which several are also peer-reviewed but were retrieved through arXiv). Venues included IEEE journals (TMI \cite{gibson2018densevnet}, Access \cite{qayyum2020aspp}), Springer LNCS (MICCAI proceedings \cite{kakeya2018ujapa}), Medical Physics \cite{zhu2019anatomynet, ma2019bladder, liu2020attention, amjad2022general}, and arXiv preprints \cite{nikolov2021clinically, han2023scribble}. This reflects both the academic rigor and the ongoing challenge of reproducibility in the field.
...
Aggregated BTCV performance from verified source papers (Supplementary Data~S8): auto-configured CNNs (nnU-Net~\cite{isensee2021nnunet}, MedNeXt~\cite{roy2023mednext}, STU-Net~\cite{huang2023stunet}) reach 85.2$\pm$2.1\%~Dice; transformers (UNETR~\cite{hatamizadeh2022unetr}, Swin UNETR~\cite{swinunetr2022}) attain 80.5$\pm$1.8\%. Auto-configured CNNs outperform pure transformers by 4.7~points. \emph{Key finding: modern auto-configured CNNs lead current BTCV benchmarks, with transformer-only approaches 4--5 points behind.}
...
The 15-question rubric (Section~\ref{sec:methodology}) was applied to the 52 included studies via a single GPT-5.2 batch call against the full text of each PDF. Aggregate results: \textbf{mean total score 21.0/30, range 14--28}. Of the 52 studies, \textbf{10 (19.2\%) were rated high quality} (total $\geq$24), \textbf{41 (78.8\%) moderate} (15--23), and \textbf{1 (1.9\%) low} ($<$15). The per-study rubric scores, full structured QA outputs, included-study list, and archived source PDFs are provided in Supplementary Report~S6 and Supplementary Data~S7.

\subsection{Sensitivity Analysis Results}

Restricting the synthesis to the 28 included studies whose primary identifier is a journal/conference DOI (the remaining 24 are arXiv-only identifiers), the key qualitative findings remained consistent: auto-configured CNNs maintained the highest BTCV Dice scores, the transformer--CNN performance gap persisted, and architectural rankings were unchanged. The proportion of high-quality studies in the DOI subset was \textbf{28.6\% (8 of 28)}, compared with \textbf{19.2\% (10 of 52)} in the full corpus---a modest increase consistent with the expectation that peer-reviewed publications apply slightly more rigorous evaluation protocols. This sensitivity analysis suggests that the inclusion of preprints did not materially alter the review's conclusions, but we note an important limitation: an arXiv identifier is only a proxy for non-peer-reviewed status, since some arXiv preprints are also published in peer-reviewed venues; we did not separately verify the publication status of each preprint.

\subsection{Reporting Bias Results}

Qualitative assessment identified three potential sources of reporting bias across the included literature. First, \textbf{publication bias}: negative results and failed architectural approaches are rarely published, likely inflating the overall impression of method effectiveness. Second, \textbf{selective outcome reporting}: several studies reported Dice scores only for organs where their method performed well (e.g., liver, spleen), omitting challenging structures (pancreas, adrenal glands); of the 52 included studies, 51 (98.1\%) had non-empty organ-level Dice entries in the supplementary extraction table and 1 (1.9\%) reported only aggregate or whole-volume metrics. A finer-grained ``fully reported all evaluated organs vs.\ partially reported'' breakdown was not derivable from the available extraction columns. Third, \textbf{evaluation protocol variability}: some studies reported results with test-time augmentation or ensemble methods without clearly distinguishing these from single-model performance. Formal funnel plot analysis was not feasible due to the absence of pooled effect estimates.
...
\textbf{Reproducibility:} Of the 52 included studies, only 13 (25.0\%) released source code (flagged as code available in the supplementary extraction table). Other reproducibility-relevant practices were more common: 48 (92.3\%) described their methods in sufficient detail to be in principle reproducible, 49 (94.2\%) documented their preprocessing pipeline, and 35 (67.3\%) reported statistical analyses (means, standard deviations, or hypothesis tests) alongside point estimates. The gap between methodological description (92.3\%) and code release (25.0\%) is the dominant reproducibility barrier.
...
\textbf{RQ4 (Clinical gaps):} Five critical barriers impede clinical translation: the \emph{vascular segmentation gap} (65--75\% vs.\ 90--96\% Dice for parenchymal organs), a \emph{reproducibility crisis} (only 25.0\% of studies released source code), \emph{domain generalization failure} (5--15\% degradation on external data), insufficient \emph{uncertainty quantification} ($<$20\% of studies address this), and unclear \emph{regulatory pathways} requiring documented failure mode characterization.
...
\textbf{Internal validity:} Selection bias mitigated by three-database search (PubMed, arXiv, Semantic Scholar), citation tracing, and AI-assisted screening with a 10\% human validation audit ($\kappa = 0.89$). Availability bias: only 39.1\% of meeting papers had accessible PDFs.
...
\textbf{COI Safeguards:} Authors A.T.N.\ and M.H.R.M.\ are DocDo-affiliated. Safeguards include LLM-based extraction (decoupling extraction from author judgement), pre-registered protocol, evidence traceability, and preservation of negative findings.
...
\textbf{Protocol:} The review protocol is available at \href{https://github.com/gameguild-gg/docdo-paper}{\faGithub\ gameguild-gg/docdo-paper}. No amendments were made to the protocol after initiation of the search and screening process.
...
\textbf{Data Availability:} All survey materials, extracted data, PRISMA screening decisions, and analysis scripts are openly available in the project repository at \href{https://github.com/gameguild-gg/docdo-paper}{\faGithub\ gameguild-gg/docdo-paper}. Supplementary artifacts referenced throughout the paper reside under \path{artifacts/data/evidence/supplementary/} and are summarized in Table~\ref{tab:supp_index}. All statistics reported in this paper can be independently validated using these materials, in particular Supplementary Data~S8 (BTCV benchmark sources with DOIs), Supplementary Report~S10 (verified statistics traceability), and Supplementary Data~S12 (per-organ performance data).

\begin{table*}[!t]
\centering
\footnotesize
\caption{Supplementary Materials Index. Files reside in \texttt{artifacts/data/evidence/supplementary/} of the project repository.}
\label{tab:supp_index}
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\textwidth}{@{}l>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
\toprule
\textbf{ID} & \textbf{File} & \textbf{Contents} \\
\midrule
S0  & \path{S0_data_provenance.md} & Data provenance and pipeline overview \\
S1  & \path{S1_search_results_REAL.csv} & Raw database search results \\
S1b & \path{S1b_citation_tracking.csv} & Forward/backward citation tracking log \\
S2  & \path{S2_final_included_studies.csv} & Final 52 included studies with full metadata \\
S3  & \path{S3_search_protocol.md} & Complete database query strings \\
S4  & \path{S4_ai_screening_protocol.md}; \newline \path{S4_screening_criteria.md} & AI screening protocol, prompts, and rubrics \\
S5  & \path{S5_screening_decisions.csv} & Per-paper screening decisions and exclusions \\
S6  & \path{S6_validation_report.md}; \newline \path{S6b_batch_processing_log.md} & Human validation audit (n=64, $\kappa$=0.89) \\
S7  & \path{S7_extraction_template.csv} & Data extraction schema and machine-readable outputs \\
S8  & \path{S8_table_sources.csv} & Per-table source mapping with DOIs \\
S9  & \path{S9_analysis_scripts.md} & Verification and reproducibility scripts (Python) \\
S10 & \path{S10_verified_statistics.md} & Traceability of every statistic in the paper \\
S11 & \path{S11_paywalled_access.md}; \newline \path{S11_per_organ_source_tracking.md} & Paywall log and per-organ source tracking \\
S12 & \path{S12_per_organ_statistics.md} & Per-organ Dice statistics (n=32 studies) \\
PRISMA & \path{S_PRISMA_funnel.md} & PRISMA flow and reporting-bias notes \\
\bottomrule
\end{tabularx}
\end{table*}
...
Confirming the foundational work of Isensee et al.~\cite{isensee2021nnunet}, our central observation is that \emph{implementation quality matters more than architectural novelty}. On BTCV, the mean Dice gap between auto-configured and earlier non-auto-configured CNNs (e.g.\ nnU-Net/MedNeXt/STU-Net at 85.2$\pm$2.1\%, $n$=3, vs.\ 3D U-Net/V-Net/Attention~U-Net at $\approx$75.7\%, $n$=3) exceeds the gap between auto-configured CNNs and pre-trained transformers (UNETR/Swin~UNETR at 80.5$\pm$1.8\%, $n$=2). These descriptive comparisons are drawn from a small number of methods per family and should not be interpreted as pooled effect sizes. Researchers should benchmark against properly-tuned baselines; practitioners should optimize systematically before chasing novel architectures.
...
\begin{enumerate}
    ...
    \item Critical gaps remain in vascular segmentation (65 to 75\% vs.\ 90 to 96\% Dice), reproducibility (25.0\% code availability), and domain generalization (5 to 15\% accuracy degradation) that must be addressed for widespread surgical planning deployment.
\end{enumerate}
