...
\textbf{Data Sources:} Three databases (PubMed via Entrez E-utilities, arXiv via the official arXiv API, and Semantic Scholar via the Graph API) were searched for studies published between 2015 and January 2026.
...
\textbf{Methodology Note:} We used an agentic screening pipeline: Elasticsearch pre-filtering followed by GPT-based consensus screening (GPT-4o-mini 3-vote unanimous-INCLUDE rule with GPT-5-nano cross-validation and GPT-5.2 tiebreaker) and a final GPT-5.2 full-text screening pass, with a human validation audit on a 10\% stratified sample (n=64 from 638 ES-filtered papers) reviewed by two domain experts blind to AI decisions and a third reviewer for discrepancies (AI--human agreement 96.4\%, $\kappa = 0.89$; 3 false negatives recovered; see \texttt{S6\_validation\_report.md}). Data extraction and quality scoring were performed by a single GPT-5.2 batch pass (rubric in \texttt{operations/src/\_archive/scripts/step2\_quality\_assessment\_batch.py}) without independent human re-extraction or re-scoring. This represents structured survey methodology with AI-assisted screening rather than a prospective dual-reviewer systematic review; the absence of human re-extraction is acknowledged as a limitation in Section~\ref{sec:limitations}.
...
Data extraction was performed by a single GPT-5.2 batch pass against the full text of each included PDF (extraction prompt in \texttt{operations/src/\_archive/scripts/step2\_quality\_assessment\_batch.py}); per-study extraction records are vendored in \texttt{pipeline\_outputs/s3\_extracted\_data\_full.csv} and the full structured LLM responses (including QA rubric output) in \texttt{pipeline\_outputs/qa\_parsed\_results.json}. There was no independent human re-extraction; field values were taken as stated in the LLM output and spot-checked by the authors against the source PDFs but not formally adjudicated by a second reviewer. Study authors were not contacted for missing or unclear data; extraction relied solely on published information. For each study, we extracted:
...
\textbf{Sensitivity analyses:} To assess robustness, we conducted one post-hoc sensitivity analysis (not pre-specified in the review protocol): restricting findings to the 26 included studies whose paper identifier is a journal/conference DOI rather than an arXiv preprint identifier (operationalised as a non-arXiv \texttt{paper\_id} in \texttt{pipeline\_outputs/qa\_parsed\_results.json}) to confirm that architectural rankings and key conclusions remained consistent.
...
\textbf{Segmentation scope:} Multi-organ segmentation was addressed by 34 studies (65.4\%), while 18 studies (34.6\%) focused on single-organ segmentation. The most frequently targeted organs (substring match against the \texttt{organs\_segmented} field of \texttt{pipeline\_outputs/s3\_extracted\_data\_full.csv}) were: liver (n=29), kidney/renal (n=26), bladder (n=20), spleen (n=17), pancreas (n=16), and stomach (n=16).
...
\textbf{Reproducibility status:} Among 52 studies, 28 (53.8\%) had a journal/conference DOI as their \texttt{paper\_id} in \texttt{pipeline\_outputs/s3\_extracted\_data\_full.csv} (the remaining 24 are arXiv-only identifiers, of which several are also peer-reviewed but were retrieved through arXiv). Venues included IEEE journals (TMI \cite{gibson2018densevnet}, Access \cite{qayyum2020aspp}), Springer LNCS (MICCAI proceedings \cite{kakeya2018ujapa}), Medical Physics \cite{zhu2019anatomynet, ma2019bladder, liu2020attention, amjad2022general}, and arXiv preprints \cite{nikolov2021clinically, han2023scribble}. This reflects both the academic rigor and the ongoing challenge of reproducibility in the field.
...
The 15-question rubric (Section~\ref{sec:methodology}) was applied to the 52 included studies via a single GPT-5.2 batch call against the full text of each PDF. Aggregate results: \textbf{mean total score 21.0/30, range 14--28}. Of the 52 studies, \textbf{10 (19.2\%) were rated high quality} (total $\geq$24), \textbf{41 (78.8\%) moderate} (15--23), and \textbf{1 (1.9\%) low} ($<$15). The per-study rubric scores are released as Supplementary~S2b (\texttt{artifacts/data/evidence/supplementary/pipeline\_outputs/qa\_summary.csv}); the full LLM responses for each rubric item are in \texttt{qa\_parsed\_results.json} alongside it. The list of 52 included studies is preserved as \texttt{S2\_final\_included\_studies.csv} and the underlying PDFs are archived under \texttt{data/final\_included\_papers/}.

\subsection{Sensitivity Analysis Results}

Restricting the synthesis to the 26 included studies whose \texttt{paper\_id} is a journal/conference DOI rather than an arXiv identifier (i.e.\ excluding the 26 arXiv-only entries in \texttt{pipeline\_outputs/qa\_parsed\_results.json}), the key qualitative findings remained consistent: auto-configured CNNs maintained the highest BTCV Dice scores, the transformer--CNN performance gap persisted, and architectural rankings were unchanged. The proportion of high-quality studies in the DOI subset was \textbf{26.9\% (7 of 26)}, compared with \textbf{19.2\% (10 of 52)} in the full corpus---a modest increase consistent with the expectation that peer-reviewed publications apply slightly more rigorous evaluation protocols. This sensitivity analysis suggests that the inclusion of preprints did not materially alter the review's conclusions, but we note an important limitation: an arXiv \texttt{paper\_id} is only a proxy for non-peer-reviewed status, since some arXiv preprints are also published in peer-reviewed venues; we did not separately verify the publication status of each preprint. Counts in this subsection were re-derived directly from the recovered per-study QA JSON in 2026-04 after the original draft cited an unverifiable ``29 studies / 10 of 29 (34.5\%)'' figure.

\subsection{Reporting Bias Results}

Qualitative assessment identified three potential sources of reporting bias across the included literature. First, \textbf{publication bias}: negative results and failed architectural approaches are rarely published, likely inflating the overall impression of method effectiveness. Second, \textbf{selective outcome reporting}: several studies reported Dice scores only for organs where their method performed well (e.g., liver, spleen), omitting challenging structures (pancreas, adrenal glands); of the 52 included studies, 51 (98.1\%) populated a non-empty \texttt{dice\_per\_organ} field in the extraction (\texttt{pipeline\_outputs/s3\_extracted\_data\_full.csv}) and 1 (1.9\%) reported only aggregate or whole-volume metrics. A finer-grained ``fully reported all evaluated organs vs.\ partially reported'' breakdown was not derivable from the LLM extraction columns, so the previous draft's claim of ``39/10/3 (75.0\%/19.2\%/5.8\%)'' has been retracted in favour of this verifiable count. Third, \textbf{evaluation protocol variability}: some studies reported results with test-time augmentation or ensemble methods without clearly distinguishing these from single-model performance. Formal funnel plot analysis was not feasible due to the absence of pooled effect estimates.
...
\textbf{Reproducibility:} Of the 52 included studies, only 13 (25.0\%) released source code (\texttt{code\_available=Yes} in \texttt{pipeline\_outputs/s3\_extracted\_data\_full.csv}). Other reproducibility-relevant practices were more common: 48 (92.3\%) described their methods in sufficient detail to be in principle reproducible, 49 (94.2\%) documented their preprocessing pipeline, and 35 (67.3\%) reported statistical analyses (means, standard deviations, or hypothesis tests) alongside point estimates. The gap between methodological description (92.3\%) and code release (25.0\%) is the dominant reproducibility barrier.
