...
Restricting the synthesis to the 26 included studies whose primary identifier is a journal/conference DOI rather than an arXiv identifier (i.e.\ excluding the 26 arXiv-only entries in the supplementary extraction metadata), the key qualitative findings remained consistent: auto-configured CNNs maintained the highest BTCV Dice scores, the transformer--CNN performance gap persisted, and architectural rankings were unchanged. The proportion of high-quality studies in the DOI subset was \textbf{26.9\% (7 of 26)}, compared with \textbf{19.2\% (10 of 52)} in the full corpus---a modest increase consistent with the expectation that peer-reviewed publications apply slightly more rigorous evaluation protocols. This sensitivity analysis suggests that the inclusion of preprints did not materially alter the review's conclusions, but we note an important limitation: an arXiv identifier is only a proxy for non-peer-reviewed status, since some arXiv preprints are also published in peer-reviewed venues; we did not separately verify the publication status of each preprint. Counts in this subsection were re-derived directly from the recovered per-study QA outputs in 2026-04 after the original draft cited an unverifiable ``29 studies / 10 of 29 (34.5\%)'' figure.

\subsection{Reporting Bias Results}

Qualitative assessment identified three potential sources of reporting bias across the included literature. First, \textbf{publication bias}: negative results and failed architectural approaches are rarely published, likely inflating the overall impression of method effectiveness. Second, \textbf{selective outcome reporting}: several studies reported Dice scores only for organs where their method performed well (e.g., liver, spleen), omitting challenging structures (pancreas, adrenal glands); of the 52 included studies, 51 (98.1\%) had non-empty organ-level Dice entries in the supplementary extraction table and 1 (1.9\%) reported only aggregate or whole-volume metrics. A finer-grained ``fully reported all evaluated organs vs.\ partially reported'' breakdown was not derivable from the available extraction columns, so the previous draft's claim of ``39/10/3 (75.0\%/19.2\%/5.8\%)'' has been retracted in favour of this verifiable count. Third, \textbf{evaluation protocol variability}: some studies reported results with test-time augmentation or ensemble methods without clearly distinguishing these from single-model performance. Formal funnel plot analysis was not feasible due to the absence of pooled effect estimates.
